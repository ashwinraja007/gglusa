...
import type { SeoRecord } from "@/api/types";

type Module = "seo"; // Only SEO module will be displayed
const modules: Module[] = ["seo"];
...
export function AdminPanel({ onLogout }: { onLogout: () => void }) {
  ...
  const { seo } = useAdminCollections(search);
  ...
  useEffect(() => {
    ...
    setSeoFormState((prev) => ({
      ...
      extra_meta_json: existing.extra_meta_json ?? {},
    }));
  ...
  const submitSeo = async (event: FormEvent) => {
    ...
    try {
      const payload = {
        ...
        extra_meta_json: seoFormState.extra_meta_json,
      };
  ...
  const onEditSeo = (record: SeoRecord) => {
    ...
    setSeoFormState({
      ...
      extra_meta_json: record.extra_meta_json ?? {},
    });
    setActive("seo");
  };
      ...
      <div className="grid gap-4 md:grid-cols-[240px_1fr]">
        ...
        <section className="space-y-4 rounded-xl border bg-white p-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            />
            <span className="text-sm text-muted-foreground">Search and manage SEO metadata for all routes.</span>
          </div>
